...
public class Vector3Float32Definition extends NumberDefinition<VectorBase> {

    public Vector3Float32Definition() {

        super(Datatype.VECTOR3F32, VectorBase.class);
    }
    ...
    @Override
    public Vector3<Float> readValue(final KaitaiStream _io) {

        return new Vector3<Float>(_io.readF4be(),
                                  _io.readF4be(),
                                  _io.readF4be());
    }
    ...
    public void writeValue(final VectorBase _value, final OutputStream _outputStream) throws
        ...
        if (_value != null) {
            Vector3<Float> vec = (Vector3<Float>)_value;
            ...
            Vector3<Float> vec = (Vector3<Float>)defaultValue;
            _outputStream.write(ByteBuffer.allocate(4).putFloat(vec.getX()).array());
            _outputStream.write(ByteBuffer.allocate(4).putFloat(vec.getY()).array());
            _outputStream.write(ByteBuffer.allocate(4).putFloat(vec.getZ()).array());
        }
